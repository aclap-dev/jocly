...
try {
	...
	exports.Game = JocGame = function JocGame() {};
	exports.Board = JocBoard = function JocBoard() {};
	exports.Move = JocMove = function JocMove() {};
} catch (e) {
...
if (typeof document != "undefined") JocGame.CLICK = 'ontouchstart' in document.documentElement ? "touchstart" : "click";else JocGame.CLICK = "click";
/*
...
JocGame.prototype.Init = function (aOptions) {
	...
	if (aOptions) {
		...
		if (typeof this.mOptions.level != "undefined") this.mTopLevel = this.mOptions.level;
		if (typeof this.mOptions.loopMax != "undefined") this.mLoopMax = this.mOptions.loopMax;
		this.mVisitedBoards = {};
		if (typeof this.mOptions.viewAs != "undefined") this.mViewAs = this.mOptions.viewAs;
	}
	...
	if (aOptions && aOptions.initial) this.GameInitGame(aOptions.initial);else this.GameInitGame();
	this.mBoard = new (this.GetBoardClass())(this);
	if (this.mBoard.InitialPosition) this.mBoard.InitialPosition(this);
	this.mBoard.mMoves = [];
...
JocGame.prototype.PlayMove = function (move) {
	...
	var promise = new Promise(function (resolve, reject) {
		...
		if (moveShown) resolve();else self.MoveShown = function () {
...
JocGame.prototype.AttachElement = function (element, options) {
	...
	this.widget = element;
	var promise = new Promise(function (resolve, reject) {
		if (game.gamePreAttachProto) reject(new Error("Game already attached"));else {
			var systemJSConfig = {
				meta: {
					"jocly-xdview.js": {
						globals: {
		...
		Promise.all([SystemJS.import("jocly-xdview.js"), SystemJS.import("games/" + game.module + "/" + game.name + "-view.js")]).then(function (args) {
			var xdview = args[0],
			    view = args[1];
...
JocGame.prototype.DetachElement = function () {
	...
	this.widget = element;
	var promise = new Promise(function (resolve, reject) {
		if (!game.gamePreAttachProto) reject(new Error("Game not attached"));else {
			// TODO
			resolve();
		}
	});
...
JocGame.prototype.DestroyView = function () {
	if (this.mWidget) this.mWidget.empty();
};
...
JocGame.prototype.UpdateSounds = function () {
	...
	if (joclySounds.length == 0) joclySounds = $("<div/>").attr("id", "jocly-sounds").css({ display: "none" }).appendTo($("body"));
	...
	for (var i in defaultSounds) {
		...
		for (var i in this.config.view.sounds) {
			...
			if (this.config.view.sounds[i]) if (this.config.view.sounds[i]) AddSound(i, this.config.baseURL + "games/" + this.config.model.module, this.config.view.sounds[i]);
		}
	}
};

JocGame.prototype.PlaySound = function (tag) {
	if (!this.CanPlaySound(tag)) return;
	...
	if (audio && this.mSounds) {
		...
		if (this.mNeedPhonegapMedia) {
			if (typeof this.mPhonegapMediaLib == "undefined") this.mPhonegapMediaLib = {};
			if (typeof this.mPhonegapMediaLib[tag] == "undefined") {
				var node = audio.firstChild;
				while (node) {
					if (/source/i.test(node.nodeName) && node.getAttribute("type") == "audio/mp3") break;
				...
				} else this.mPhonegapMediaLib[tag] = null;
			}
			if (this.mPhonegapMediaLib[tag]) {
				this.mPhonegapMediaLib[tag].play();
			}
		} else audio.cloneNode(true).play();
	}
};
...
JocGame.prototype.GameInitGame = function () {
	if (this.mGameInited == false) {
		this.mVisitedBoards = {};
		if (arguments.length > 0 && arguments[0]) this.mInitial = arguments[0];else this.mInitial = null;
		this.InitGame();
...
JocGame.prototype.DisplayBoard = function () {
	if (this.mBoard.Display) this.mBoard.Display(this);
};
...
JocGame.prototype.EvaluateBoard = function () {
	...
	this.mCurrentLevel = -1;
	this.mBoard.GenerateMoves(this);
	if (this.mBoard.mFinished == false) this.mBoard.Evaluate(this, true, true);
...
JocGame.prototype.GetFinished = function () {
	...
	this.SetWho(-this.mWho);
	if (this.mBoard.mFinished) return this.mBoard.mWinner;else return 0;
};
...
JocGame.prototype.AddVisit = function (board, sign) {
	if (board) sign = board.GetSignature();
	var visits = this.mVisitedBoards[sign];
	if (visits === undefined) this.mVisitedBoards[sign] = 1;else this.mVisitedBoards[sign]++;
};
...
JocGame.prototype.RemoveVisit = function (board, sign) {
	...
	var visits = this.mVisitedBoards[sign];
	if (visits !== undefined) {
		if (visits > 1) this.mVisitedBoards[sign]--;else delete this.mVisitedBoards[sign];
	}
};
...
JocGame.prototype.StartMachine = function (aOptions) {
	...
	if (typeof aOptions.level != "undefined") this.mTopLevel = aOptions.level;
	if (typeof aOptions.maxDepth != "undefined") this.mTopLevel = aOptions.maxDepth;
	...
	this.mAborted = false;
	this.mRandomSeed = 0;
	if (aOptions.randomSeed && !isNaN(parseInt(aOptions.randomSeed))) this.mRandomSeed = parseInt(aOptions.randomSeed);
	...
	if (this.mOptions.levelOptions) {
		...
		if (aOptions.level) Object.assign(this.mOptions.levelOptions, aOptions.level);
	}

	var aiThread = aOptions.threaded && (typeof window === "undefined" ? "undefined" : _typeof(window)) == "object" && window.Worker;
	if (aOptions.level && aOptions.level.ai == "uct" && JoclyUCT) {
		if (aiThread) this.StartThreadedMachine(aOptions, "uct");else JoclyUCT.startMachine(this, aOptions);
	} else {
		// default is legacy alpha-beta ai
		if (aiThread) this.StartThreadedMachine(aOptions, "alpha-beta");else {
...
JocGame.prototype.Random = function (roof) {
	var value;
	if (this.mRandomSeed) value = this.mRandomSeed % roof;else value = Math.floor(Math.random() * roof);
	return value;
};
...
JocGame.prototype.ArrayShuffle = function (arr) {
	...
	while (--i) {
		var j;
		if (this.mRandomSeed) j = this.mRandomSeed % (i + 1);else j = Math.floor(Math.random() * (i + 1));
		var tmp = arr[i];
		arr[i] = arr[j];
		arr[j] = tmp;
	}
...
JocGame.prototype.Done = function () {
	...
	if (this.mSavedVisitedBoards) this.mVisitedBoards = this.mSavedVisitedBoards;
...
JocGame.prototype.Engine = function (aBoard, aLevel, aBAlpha, aAlpha, aPotential) {
	...
	if (context.mBoard.mMoves.length == 0) context.mBoard.GenerateMoves(this);
	...
	if (context.mBoard.QuickEvaluate) {
		var MoveSort = function MoveSort(bm1, bm2) {
		...
		context.mNextBoards = [];
		if (typeof this.mOptions.capMoves != "undefined") boardsMoves = boardsMoves.slice(0, this.mOptions.capMoves);
...
JocGame.prototype.ExecuteStep = function () {
	...
	if (this.mProgressCallback) {
		var percent = null;
		if (context.mLevel == this.mTopLevel) percent = Math.floor(context.mMoveIndex * 100 / context.mBoard.mMoves.length);else if (context.mLevel == this.mTopLevel - 1) {
	...
	if (nextBoard.mFinished) {
		...
		if (potential >= 1) {
			...
			this.Engine(nextBoard, context.mLevel - 1, context.mMoveIndex != 0, context.mBestEvaluation, potential); // recurse algo
			return;
		}
	}
	this.ExecuteStep2();
...
JocGame.prototype.ExecuteStep2 = function () {
		...
		if (context.mMoveIndex == 0) {
			...
			if (context.mNextBoard.mWho > 0) {
				...
				if (context.mNextBoard.mEvaluation > context.mBestEvaluation) {
					...
					if (context.mLevel == this.mTopLevel) // if top level
						this.SetBest(context.mBoard.mMoves[context.mMoveIndex], context.mBoard); // then store
				} else if (context.mLevel == this.mTopLevel && context.mNextBoard.mEvaluation == context.mBestEvaluation) {
					// top level and
					// another best
					// move
					this.AddBest(context.mBoard.mMoves[context.mMoveIndex], context.mBoard); // add to best moves
				}
			} else {
				// A plays
				if (context.mNextBoard.mEvaluation < context.mBestEvaluation) {
					...
					if (context.mLevel == this.mTopLevel) this.SetBest(context.mBoard.mMoves[context.mMoveIndex], context.mBoard);
				} else if (context.mLevel == this.mTopLevel && context.mNextBoard.mEvaluation == context.mBestEvaluation) this.AddBest(context.mBoard.mMoves[context.mMoveIndex], context.mBoard);
			}
		}
	}
	...
	if (context.mBAlpha) {
		// alpha-beta pruning
		if (context.mBoard.mWho == JocGame.PLAYER_A && context.mBestEvaluation > context.mAlpha || context.mBoard.mWho == JocGame.PLAYER_B && context.mBestEvaluation < context.mAlpha) {
...
JocGame.prototype.GetRepeatOccurence = function (board) {
	if (!this.mOptions.preventRepeat) return -1;
...
JocGame.prototype.HandleRepeat = function (board) {
	if (this.mOptions.preventRepeat) {
		...
		if (this.mVisitedBoards[sign] === undefined) this.mVisitedBoards[sign] = 1;else this.mVisitedBoards[sign]++;
	}
};
...
JocGame.prototype.UnhandleRepeat = function (board) {
	if (this.mOptions.preventRepeat) {
		var sign = board.GetSignature(true);
		if (this.mVisitedBoards[sign] == 1) delete this.mVisitedBoards[sign];else if (this.mVisitedBoards[sign] > 1) this.mVisitedBoards[sign]--;
	}
};
...
JocGame.prototype.ApplyMove = function (aMove) {
	...
	if (this.mFullPlayedMoves.length < this.mPlayedMoves.length) this.mFullPlayedMoves.push(move);else if (!move.Equals(this.mFullPlayedMoves[this.mPlayedMoves.length - 1])) {
...
JocGame.prototype.BackTo = function (aIndex, moves) {
	if (!moves) moves = this.mFullPlayedMoves;
	this.mWho = JocGame.PLAYER_A;
	this.mBoard = new (this.GetBoardClass())(this);
	if (this.mBoard.InitialPosition) this.mBoard.InitialPosition(this);
	if (this.mInitial && this.mInitial.turn) this.mWho = this.mInitial.turn;
	this.mBoard.mWho = this.mWho;
	this.mBestMoves = [];
	this.mVisitedBoards = {};
	this.mPlayedMoves = [];
...
JocGame.prototype.Load = function (gameData) {
	...
	if (gameData.initialBoard) board.CopyFrom(gameData.initialBoard);
	this.mBestMoves = [];
	this.mVisitedBoards = {};
	...
	for (var i in moves) {
		...
		if (!this.IsValidMove(move)) throw "invalid-move";
...
JocBoard.prototype.IsValidMove = function (aGame, move) {
	if (typeof move.Equals != "function") move = aGame.CreateMove(move);
	if (!this.mMoves || this.mMoves.length == 0) {
		this.mCurrentLevel = -1;
		this.GenerateMoves(aGame);
	}
	for (var i in this.mMoves) {
		if (move.Equals(this.mMoves[i])) return true;
	}
...
JocGame.prototype.GetBestMatchingMove = function (moveStr, candidateMoves) {
	...
	candidateMoves.forEach(function (m) {
		if (typeof m.ToString == "function") prettyMoves.push(m.ToString());else prettyMoves.push($this.CreateMove(m).ToString());
	});
	...
	candidateMoves.forEach(function (candidate, index) {
		...
		if (dist == bestDist) bestMatches.push(index);else if (dist < bestDist) {
			bestMatches = [index];
			bestDist = dist;
		}
	});
	if (bestMatches.length == 1) return candidateMoves[bestMatches[0]];

	var candidateIndexes = bestMatches;
	var matches = [];
	candidateIndexes.forEach(function (index) {
		var pretty = prettyMoves[index];
		if (moveStr.indexOf(pretty) >= 0 || pretty.indexOf(moveStr) >= 0) matches.push(index);
	});
	if (matches.length == 1) return candidateMoves[matches[0]];
...
JocBoard.prototype.PickMoveFromDatabase = function (aGame, database) {
	...
	if (this.mMoves.length == 0) return null;
...
JocBoard.prototype.CompactMoveString = function (aGame, aMove) {
	if (typeof aMove.ToString != "function") aMove = aGame.CreateMove(aMove);
...
JocGame.Zobrist.prototype = {
	update: function update(zobrist, name) {
...
JocGame.Levenshtein = function (e, f) {
	if (e == f) return 0;var d = e.length,
	    j = f.length;if (0 === d) return j;if (0 === j) return d;var b = !1;try {
		b = !"0"[0];
	} catch (m) {
		b = !0;
	}
	b && (e = e.split(""), f = f.split(""));for (var b = Array(d + 1), g = Array(d + 1), a = 0, h = 0, i = 0, a = 0; a < d + 1; a++) {
		b[a] = a;
		...
		for (a = 0; a < d; a++) {
			var c = e[a],
			    ...
			    c = b[a + 1] + 1,
			    l = g[a] + 1,
			    i = b[a] + i;l < c && (c = l);i < c && (c = i);g[a + 1] = c;
		}a = b;b = g;g = a;
	}return b[d];
};
