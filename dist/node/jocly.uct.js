...
} else undefined.JoclyUCT = JoclyUCT;
...
(function () {
	...
	Node.prototype = {
		addParent: function addParent(parent) {
			...
			if (parent.depth + 1 < this.depth) this.depth = parent.depth + 1;
		}
	};
	...
	JoclyUCT.startMachine = function (aGame, aOptions) {
		...
		if (uctParams.log) console.log("Running UCT AI - ", aOptions.level.label, "- Player", aGame.mWho == 1 ? "A" : "B");
   ...
   */
		function GetMinimaxEval(node, children) {
			...
			for (var i = 0; i < children.length; i++) {
				...
				if (evaluation === undefined || node1.evaluation * node1.who > evaluation * node1.who) evaluation = node1.evaluation;
			}
			return evaluation;
		}
		/*
   ...
   */
		function GetProductEval(node, children) {
			...
			for (var i = 0; i < children.length; i++) {
				...
				if (node.who == 1) value *= 1 - node1.evaluation;else value *= node1.evaluation;
			}
			if (node.who == 1) return (1 - value) * 2 - 1;else return value * 2 - 1;
		}

		function PropagateEvalParent(node, visits, visited) {
			if (aGame.mOptions.uctTransposition && !aGame.mOptions.uctIgnoreLoop && node.sign in visited) return;
			var children = [];
			if (uctParams.ignoreLeaf) {
				...
				for (var i = 0; i < node.children.length; i++) {
					...
					for (var i = 0; i < node.children.length; i++) {
						...
						if (node1.known || node1.children) children.push(node1);
					}
				}
			}
			if (children.length == 0) for (var i = 0; i < node.children.length; i++) {
			...
			switch (uctParams.propagation) {
				...
				case "minimax2min-best":
					var evaluation1 = GetMaximinEval(node, children);
					var evaluation2 = GetMinimaxEval(node, children);
					if (node.who == 1) evaluation = Math.max(evaluation1, evaluation2);else evaluation = Math.min(evaluation1, evaluation2);
					break;
				case "product":
				...
				default:
					...
					if (uctParams.useDepthWeights) evaluation = WeightEval(evaluation, node.depth + 1);
			}
			...
			if (node.evaluation !== evaluation) {
				...
				if (!uctParams.directVisits) node.visits += visits;
				PropagateEval(node, visits, visited);
			} else if (!uctParams.directVisits) PropagateVisits(node, visits, visited);
		}
   ...
   */
		...
		function PropagateKnown(node, visited) {
			if (node.known == false || node.parents.length == 0) return;
			if (aGame.mOptions.uctTransposition && !aGame.mOptions.uctIgnoreLoop) {
				if (!visited) visited = {};
				visited[node.sign] = true;
			}
   ...
   */
		function Step() {
			...
			var beta = 2;
			while (true) {
				var PickBestChildren = function PickBestChildren() {
					for (var i = 0; i < candidateChildren.length; i++) {
						var child1 = candidateChildren[i];
						var node1 = child1.n;
						if (node1.known) continue;
						var value = (node1.evaluation * node1.who + 1) / 2; // ensures value between 0 and 1
						var ucb;
						if (uctParams.directVisits) ucb = value + uctParams.c * Math.sqrt(parentVisitsLog / child1.f);else ucb = value + uctParams.c * Math.sqrt(parentVisitsLog / node1.visits);
						if (bestChildren.length == 0 || ucb >= bestUCB) {
							if (bestChildren.length > 0 && ucb > bestUCB) bestChildren = [];
							bestUCB = ucb;
     ...
     */
				...
				var PickBestChildrenDistributeEval = function PickBestChildrenDistributeEval() {
					...
					for (var vi = 0; vi < values.length; vi++) {
						...
						for (var i = 0; i < children.length; i++) {
							...
							var node1 = child1.n;
							var ucb;
							if (uctParams.directVisits) ucb = value1 + uctParams.c * Math.sqrt(parentVisitsLog / child1.f);else ucb = value1 + uctParams.c * Math.sqrt(parentVisitsLog / node1.visits);
							if (bestChildren.length == 0 || ucb >= bestUCB) {
								if (bestChildren.length > 0 && ucb > bestUCB) bestChildren = [];
								bestUCB = ucb;
								bestChildren.push(child1);
				...
				if (uctParams.useAlphaBeta) {
					...
					for (var i = 0; i < node.children.length; i++) {
						...
						node1.evaluation > alpha) alpha = node1.evaluation;
						if (node1.who == -1 && // minimizing player
						node1.evaluation < beta) beta = node1.evaluation;
						if (beta < alpha) {
				...
				} else candidateChildren = node.children;
				if (aGame.mOptions.uctTransposition && !aGame.mOptions.uctIgnoreLoop) {
					var candidateChildren0 = [];
					for (var i = 0; i < candidateChildren.length; i++) {
						if (!(candidateChildren[i].n.sign in visited)) candidateChildren0.push(candidateChildren[i]);
					}candidateChildren = candidateChildren0;
				}
				var bestChildren = [],
				    bestUCB;
				var parentVisitsLog;
				if (uctParams.directVisits) parentVisitsLog = Math.log(parentVisits);else parentVisitsLog = Math.log(node.visits);

				if (uctParams.distributeEval) PickBestChildrenDistributeEval();else PickBestChildren();
				...
				node = child.n;
				if (aGame.mOptions.uctTransposition && !aGame.mOptions.uctIgnoreLoop) visited[node.sign] = 1;
				depth++;
				moves.push(child.m);
				...
				if (aGame.mOptions.uctTransposition == "states") pathSign ^= TransformInteger(board.GetSignature()); // consider the states we have been through but not their order
				board.mWho = -board.mWho;
			}

			// Expand
			if (node == rootNode || node.visits >= uctParams.minVisitsExpand) {
				if (!board.mMoves || board.mMoves.length == 0) board.GenerateMoves(aGame);
				if (board.mFinished) {
					...
					for (var i = 0; i < board.mMoves.length; i++) {
						...
						} else if (aGame.mOptions.uctTransposition == "state") sign1 = signature; // only final state counts
						var node1 = null;
						if (aGame.mOptions.uctTransposition) node1 = uctNodes[sign1];
						if (!node1) {
							...
							if (board1.mFinished) {
								...
								if (isNaN(board1.mEvaluation)) console.error("Evaluation in not a number !", board1.mEvaluation);
						...
						if (node1.known == false) known = false;
						var nodeChain = {
							n: node1,
							m: new (aGame.GetMoveClass())(move).Strip() };
						if (uctParams.directVisits) nodeChain.f = 1;
						node.children.push(nodeChain);
						var _eval = node1.evaluation * node1.who;
						if (bestEval === undefined || _eval > bestEval * node1.who) bestEval = node1.evaluation;
					...
					if (uctParams.directVisits) for (var i = 0; i < nodePath.length; i++) {
			...
			function Playout(node, board, signatures) {
				...
				for (var depth = 0; depth < uctParams.playoutDepth || board.mWho == uctParams.playoutCeil * rootNode.who; depth++) {
					...
					for (var i = 0; i < board.mMoves.length; i++) {
						...
						if (board1.mFinished) {
							if (board1.mWinner == 1) evaluation = Number.MAX_VALUE;else if (board1.mWinner == -1) evaluation = -Number.MAX_VALUE;else evaluation = 0;
						} else if (isNaN(board1.mEvaluation)) console.error("Evaluation in not a number !");
      ...
      */
				...
				var nodeEval;
				if (result.finished) nodeEval = result.winner; // 1, -1 or 0
				else {
						if (uctParams.debugRawEval) node.rawEval = result.eval;
   ...
   */
		...
		if (aGame.mBoard.mMoves.length == 0) aGame.mBoard.GenerateMoves(aGame);
		...
		nodeCount++;
		if (aGame.mOptions.uctTransposition) uctNodes[aGame.mBoard.GetSignature()] = rootNode;
		...
		function Run() {
			...
			if (uctParams.maxDuration > 0) progressPercent = Math.round(100 * (now - t0) / (uctParams.maxDuration * 1000));
			if (uctParams.maxLoops > 0) progressPercent = Math.max(progressPercent, 100 * loopCount / uctParams.maxLoops);
			if (uctParams.maxNodes > 0) progressPercent = Math.max(progressPercent, 100 * nodeCount / uctParams.maxNodes);
			if (progressPercent != lastProgressPercent) {
				lastProgressPercent = progressPercent;
				if (aGame.mProgressCallback) aGame.mProgressCallback(progressPercent);
			}
			if (!rootNode.children || rootNode.known == false && (uctParams.maxDuration <= 0 || now < uctParams.maxDuration * 1000 + t0) && (uctParams.maxLoops <= 0 || loopCount < uctParams.maxLoops) && (uctParams.maxNodes <= 0 || nodeCount < uctParams.maxNodes)) {
				do {
					signatures = [];
					try {
						Step();
				...
				if (uctParams.pickMove == "maxvisits" && uctParams.directVisits) {
					...
					if (aGame.mBestMoves.length == 0) {
						for (var i = 0; i < rootNode.children.length; i++) {
							...
							if (bestEval === undefined || bestEval <= child.f) {
								if (bestEval === undefined || bestEval < child.f) aGame.mBestMoves = [];
								bestEval = child.f;
								aGame.mBestMoves.push(child.m);
							}
						}
					...
					var candidateChildren = [];
					if (uctParams.pickMove == "besteval-multivisits") rootNode.children.forEach(function (child) {
						if (child.n.visits > 1 || child.n.known == true) candidateChildren.push(child);
					});
					if (candidateChildren.length == 0) candidateChildren = rootNode.children;
					for (var i = 0; i < candidateChildren.length; i++) {
						...
						if (bestEval === undefined || bestEval >= node.evaluation * rootNode.who) {
							if (bestEval === undefined || bestEval > node.evaluation * rootNode.who || bestEval == node.evaluation * rootNode.who && (bestEval2 === undefined || bestEval2 > rootNode.who * staticEval)) {
		...
		function ReportStats(node) {
			...
			function ShowMinimax(node, depth) {
				...
				for (var i = 0; i < node.children.length; i++) {
					var child1 = node.children[i];
					var node1 = child1.n;
					console.log(indent, "  " + (node1.evaluation == node.evaluation ? "*" : " ") + " move", new (aGame.GetMoveClass())(child1.m).ToString(), "visits", node1.visits, "eval", node1.evaluation, "known", node1.known, "sev", node1.staticEvalSum + "/" + node1.staticEvalCount, "who", node1.who, "children", node1.children ? node1.children.length : "no");
					if (node1.children && node1.evaluation == node.evaluation) ShowMinimax(node1, depth + 1);
				}
			}
			...
			if (uctParams.checkSide) {
				var CheckSide = function CheckSide(node) {
					checkSideNodeCount++;
					if (node.children) for (var i = 0; i < node.children.length; i++) {
						var child1 = node.children[i];
						if (child1.n.who != -node.who) checkSideError++;
						CheckSide(child1.n);
					}
				};
...
//# sourceMappingURL=jocly.uct.js.map
