...
function receiveMessage(event) {
    ...
    switch (message.type) {
        case "init":
            ...
            Jocly.createInternalGame(message.gameName).then(function (game) {
                gGame = game;
                game.AddListener(Listen);
                function Start() {
                    game.AttachElement(area).then(function () {
                        game.GameInitView();
                        game.DisplayBoard();
                        Reply();
                    });
...
function Listen(message) {
    switch (message.type) {
        ...
        case "machine-move":
            ...
            gGame.PlayMove(move).then(function () {
                var finished = gGame.GetFinished();
                if (!finished) gGame.InvertWho();
                gGame.DisplayBoard();
                window.parent.postMessage({
...
//# sourceMappingURL=jocly.embed.js.map
