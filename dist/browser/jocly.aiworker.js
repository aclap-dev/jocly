...
onmessage = function onmessage(e) {
    ...
    switch (message.type) {
        case "Init":
            var scripts = [message.baseURL + "jocly.core.js"];
            Jocly = self.exports = {};
            importScripts.apply(null, scripts);
            importTime = Date.now() - t0;
            break;
        case "Play":
            Jocly.createInternalGame(message.gameName, options).then(function (game) {
                game.mBoard.mMoves = [];
                game.Load({
                ...
                if (typeof options.level != "undefined") game.mTopLevel = options.level;
                if (typeof options.maxDepth != "undefined") game.mTopLevel = options.maxDepth;
                ...
                game.mAborted = false;
                game.mRandomSeed = 0;
                if (options.randomSeed && !isNaN(parseInt(options.randomSeed))) game.mRandomSeed = parseInt(options.randomSeed);
                if (game.mOptions.levelOptions) {
                    game.mOptions.levelOptionsSaved = JSON.parse(JSON.stringify(game.mOptions.levelOptions));
                    if (options.level) JocUtil.extend(game.mOptions.levelOptions, options.level);
                }
                switch (e.data.algo) {
                    case "uct":
...
//# sourceMappingURL=jocly.aiworker.js.map
