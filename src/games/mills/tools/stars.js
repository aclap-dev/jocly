...
var ephem=new Ephemeris();
  var date = new Date(1967,6,14,4,0,0,0);
  var observers_longitude = 3.0408;
  var observers_latitude = 43.6528;
  var starsdb=[];
  
  var starCount=500;
  
  for(var i=0;i<stars.length && i<starCount;i++) {
  	var position = ephem.to_horizontal(date,observers_longitude,observers_latitude,stars[i].RAh,stars[i].DEd);
  	starsdb.push({
  		az: position.azimuth,
  		al: position.altitude,
  		//ma: stars[i].Vmag,
  		//rah: stars[i].RAh, 
  		//ded: stars[i].DEd,
  	});
  ...
  console.log("var url=document.currentScript;");
  console.log("if(!url) { var scripts=document.getElementsByTagName('script'); url=scripts[scripts.length-1].src; }");
  console.log("$(document).trigger('jocly.json-resource',{ url: url,data: ");
  console.log(starsdb);
  console.log("})");
  console.log("})()");
